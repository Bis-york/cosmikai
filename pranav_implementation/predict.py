...
def load_light_curve_details(request: TargetRequest, *, min_points: int = 2000) -> LightCurveDetails:
    ...
    # Increase astroquery timeout to handle slow MAST archive connections
    try:
        from astroquery.mast import Observations
        if hasattr(Observations, 'TIMEOUT'):
            Observations.TIMEOUT = 1200  # Increase to 20 minutes  # type: ignore
            _log("Increased MAST archive timeout to 1200 seconds")
    except Exception as e:
        _log(f"Could not adjust timeout: {e}")
    
    # Try searching with retry logic
    ...
    for attempt in range(max_retries):
        try:
            _log(f"Search attempt {attempt + 1}/{max_retries}...")
            search = lk.search_lightcurve(str(request.target), mission=request.mission, author=request.author)
            break
        except TimeoutError as e:
            if attempt < max_retries - 1:
                _log(f"Timeout occurred, retrying... ({e})")
                time_module.sleep(5)  # Wait 5 seconds before retry
            else:
                raise TimeoutError(
                    f"Failed to fetch light curve for {request.target} after {max_retries} attempts. "
                    f"The MAST archive may be slow or unresponsive. Try again later or use a different target."
                ) from e
    ...
    _log(f"Found {len(search)} light curve file(s); downloading and stitching")
